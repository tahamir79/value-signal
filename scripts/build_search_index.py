...
def bm25_search(index: dict[str, Any], query: str, ticker: str | None = None, limit: int = 5, k1: float = 1.5, b: float = 0.75) -> list[dict[str, Any]]:
    ...
    return [{**index["documents"][doc_id], "score": round(score, 6), "matchedTerms": [term for term in terms if any(row[0] == doc_id for row in index["postings"].get(term, []))]} for doc_id, score in sorted(scores.items(), key=lambda item: (-item[1], item[0]))[:limit]]
    ...
    for security in build_universe(args.limit):
        try:
            for filing in provider.fetch_recent(security.ticker, security.cik, args.per_form):
                chunks.extend(chunk_filing(filing, clean_filing_html(filing.html)))
