...
def audit(data_dir: Path = Path("public/data")) -> list[str]:
    ...
    for row in dashboard["records"]:
        ...
        dates = [bar["date"] for bar in row["priceHistory"]]
    ...
    values = [(bar.get("adjusted_close") or bar["close"]) for bar in sample["priceHistory"][-253:]]
    ...
    for row in dashboard["records"]:
        ticker = row["security"]["ticker"]
        facts = row["latestFacts"]
        checks = {
            "price": row["priceHistory"][-1].get("adjusted_close") or row["priceHistory"][-1]["close"],
    ...
    if warning_count:
        failures.append(f"range warnings: {warning_count}")
