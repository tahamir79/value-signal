...
- The scheduled refresh now uses 5 ETL chunks of 250 tickers per run and 6 weekday sweep slots. That gives 1,250 attempted rows per run and enough daily sweep capacity to cover the roughly 6,000-row active universe even if one scheduled window fails, while the shared `value-signal-etl` concurrency group keeps artifact-writing jobs serialized.
...
python scripts/pipeline/refresh_public_batch.py --universe data/universe/universe.json --batch-size 250 --batch-count 5 --skip-backtest
python scripts/benchmark_growth_spurt.py
